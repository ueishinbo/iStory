...
public class AllFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        SharedPre tagGetter=new SharedPre();
        tagGetter.saveTagInfo(getContext());
        List<String> sectionTypes=tagGetter.getTagInfo(getContext());

        fragments = new ArrayList<>();
        for(int i=0;i<sectionTypes.size();i++){
            fragments.add(new SectionBodyFragment(sectionTypes.get(i)));
        }
